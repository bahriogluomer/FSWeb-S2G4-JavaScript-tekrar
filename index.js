...
function compareFn(a, b) {
  ...
  const siralisayilar = besyuzdenkucuksayilar.sort(compareFn);
  console.log(siralisayilar);

  //3f çözümü

function repeatedNumbers(x) {
  ...
  for (let sayi in tekraredensayilarObjesi) {
    tekraredensayilar.push(
      `${sayi} sayısı ${tekraredensayilarObjesi[sayi]} kere tekrar edilmiştir`
    );
  }
